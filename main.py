...
def main() -> None:
    ...
    input_path = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / "inputs" / "lnad_input.json"
    output_path = Path(sys.argv[2]) if len(sys.argv) > 2 else ROOT / "inputs" / "_outputs" / "lnad_report.md"

    locator = LLMLocator(
        provider=env_str("FD_REPORTAI_LLM_PROVIDER", "local"),
