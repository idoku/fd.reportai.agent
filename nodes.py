...
from tools import *
from app.utils.text_utils import extract_user_prompt,extract_clean_json

mainLLM = LLMFactory.get_llm(model_type="qwen")  # 你已有的调用本地 LLM 的方法
llm = mainLLM.llm
...
def create_planner_node(state: State):
    ...
    content = extract_user_prompt(response['content'])
    jsonRaw  = extract_clean_json(content)
    plan = json.loads(jsonRaw)
...
def update_planner_node(state: State):
    ...
    while True:
        try:
            ...
            response = response.model_dump_json(indent=4, exclude_none=True)
            response = json.loads(response)
            plan = json.loads(extract_json(extract_user_prompt(response['content'])))
            state['messages']+=[AIMessage(content=json.dumps(plan, ensure_ascii=False))]
            return Command(goto="execute", update={"plan": plan})
